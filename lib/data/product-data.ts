...
import { PRODUCTS_TAG } from "../constants";
...
function transformProductData(prismaProduct: any) {
  ...
  const specifications =
    prismaProduct.specifications?.reduce(
      (acc: Record<string, string>, spec: any) => {
        if (spec.specificationDef) {
          acc[spec.specificationDef.name] = spec.value;
        }
        return acc;
      },
      {}
  ...
  return {
    ...
    offerEndsIn: activeDiscount?.discount?.expiresAt
      ...
      : undefined,
    variantTypes: prismaProduct.hasVariants ? variantTypes : undefined,
